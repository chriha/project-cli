...
class Processor
{
    ...
    protected function execute(string $title, Process $process, Closure $callback = null) : int
    {
        ...
        $process->start(
            function ($type, $output) use ($callback)
            {
        ...
        $this->output->writeln(
            "<options=bold><comment>[{$this->connection->getEnvironment()}]</comment>: {$title}</>"
        );
        ...
        if ( ! $process->isSuccessful() && $this->break) {
    ...
    protected function getFrames()
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            $frames[] = "[" . str_repeat(" ", $left) . "{$ball}" . str_repeat(
                    " ",
                    $length - ($left++)
                ) . "]";
        }
    ...
    protected function addOutput($content, bool $success) : void
    {
        ...
        foreach ($lines as $line) {
            ...
            if ($success) {
                $this->output->writeln(
                    '<comment>[' . $this->connection->getEnvironment() . ']</comment>: '
                    . trim($line)
                );
            } else {
                $this->output->writeln(
                    '<comment>[' . $this->connection->getEnvironment() . ']</comment>: <fg=red>'
                    . trim($line) . '</>'
                );
            }
        }
