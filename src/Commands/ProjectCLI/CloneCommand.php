...
class CloneCommand extends Command
{
    ...
    public function handle() : void
    {
        ...
        $this->spinner(
            'Cloning repository',
            new Process(['git', 'clone', '-q', $repository, $directory])
        );

        if ( ! $this->getApplication()->has('install') && ! $this->getApplication()->has('setup')) {
            return;
        }

        if ( ! $this->ask('Would you like to install the project?', 'no')) {
            return;
        }
